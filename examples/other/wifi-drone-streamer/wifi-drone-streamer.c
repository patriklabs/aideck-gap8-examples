...
static CPXPacket_t rxp_setpoint;
void rx_control_task(void *parameters)
{
  while (1)
  {
    cpxReceivePacketBlocking(CPX_F_CONTROL, &rxp_setpoint);

    // cpxPrintToConsole(LOG_TO_CRTP, "rec control pack \n");
...
void sendBufferViaCPX(CPXPacket_t *packet, uint8_t *buffer, uint32_t bufferSize)
{
  ...
  uint32_t size = 0;
  uint8_t magic = 0xAC;
  uint16_t packetSize = sizeof(packet->data) - 1;
  do
  {
    packet->data[0] = magic;
    size = packetSize;
    ...
    memcpy(&packet->data[1], &buffer[offset], size);
    packet->dataLength = size + 1;
    cpxSendPacketBlockingThreadSafe(packet);
    offset += size;
  ...
  while (1)
  {
    if (wifiClientConnected == 1)
    {
      ...
      if (streamerMode == JPEG_ENCODING)
      {
        ...
        // First send header
        txp.data[0] = 0xAC;
        memcpy(&txp.data[1], header.data, headerSize);
        txp.dataLength = headerSize + 1;
        cpxSendPacketBlockingThreadSafe(&txp);
        ...
        // Send footer
        txp.data[0] = 0xAC;
        memcpy(&txp.data[1], footer.data, footerSize);
        txp.dataLength = footerSize + 1;
        cpxSendPacketBlockingThreadSafe(&txp);
...
void start_example(void)
{
  ...
  cpxEnableFunction(CPX_F_MYAPP);
  cpxEnableFunction(CPX_F_CONTROL);
  ...
  xTask = xTaskCreate(rx_control_task, "rx_control_task", configMINIMAL_STACK_SIZE * 2,
                      NULL, tskIDLE_PRIORITY + 1, NULL);

  if (xTask != pdPASS)
  {
    cpxPrintToConsole(LOG_TO_CRTP, "RX control task did not start !\n");
    pmsis_exit(-1);
  }

  xTask = xTaskCreate(imu_transmit_task, "imu_transmit_task", configMINIMAL_STACK_SIZE * 2,
                      NULL, tskIDLE_PRIORITY + 1, NULL);

  if (xTask != pdPASS)
  {
    cpxPrintToConsole(LOG_TO_CRTP, "imu task did not start !\n");
    pmsis_exit(-1);
  }
